...
PLUGIN_FILE="$DIR/banimal-ecosystem-connector.php"
README_FILE="$DIR/readme.txt"

header_version=$(grep -m1 '^ \* Version:' "$PLUGIN_FILE" | sed -E 's/.*Version:[[:space:]]+([0-9][0-9A-Za-z.\-]*).*/\1/')
constant_version=$(grep -m1 "define('BANIMAL_CONNECTOR_VERSION'" "$PLUGIN_FILE" | sed -E "s/.*define\('BANIMAL_CONNECTOR_VERSION', '([^']+)'\).*/\1/")
...
echo "BANIMAL_CONNECTOR_VERSION:   $constant_version"
