...
(function () {
  ...
  function updateAriaLabel() {
    const key = d === 'dark' ? 'a11y.themeToLight' : 'a11y.themeToDark';
    t.setAttribute('aria-label', window.foxI18n ? window.foxI18n.t(key) : 'Switch to ' + (d === 'dark' ? 'light' : 'dark') + ' mode');
  }
  updateAriaLabel();
  window.foxI18n && window.foxI18n.onChange(updateAriaLabel);
...
(function () {
  ...
  function setRaised(state) {
    ...
    pill.textContent = raised ? t('flag.pill.active', 'Flag activated — ecosystem live') : t('flag.pill.standing', 'Standing by');
    btn.textContent = raised ? t('flag.btn.lower', 'Lower the flag') : t('flag.btn.raise', 'Raise the flag');
  }

  setRaised(false);
  window.foxI18n && window.foxI18n.onChange(() => setRaised(raised));
